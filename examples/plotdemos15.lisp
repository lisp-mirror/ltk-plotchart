...
(defparameter *xscale* '(0 0.1 0.1))       ;# X axis is 3-d depth
...
(with-ltk ()
          (wm-title *tk* "3D Ribbon Plotchart")
          ;; example with full 3D axes
          (let* ((c1 (make-instance 'canvas :width 800 :height 400
                                    :background "#aaeeff"))
                 (s1 (chart:create-3d-plot c1 *xscale* *yscale* *zscale*)))
            (configure c1 :border 0 :highlightthickness 0)
            (pack c1 :side :top :fill :both :expand t)
            (chart:ribbon s1 *YZ*))
          ;; example with minimal axes
          (let* ((c2 (make-instance 'canvas :width 800 :height 400
                                    :background "#aaeeff"))
                 (s2 (chart:create-3d-ribbon-plot c2 *yscale* *zscale*)))
            (configure c2 :border 0 :highlightthickness 0)
            (pack c2 :side :top :fill :both :expand t :pady "{1 0}")
            (chart:plot s2 *YZ*))
